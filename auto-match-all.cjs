...
async function run() {
  ...
  const { data: allTx } = await supabase.from('bank_transactions').select('*').eq('building_id', BID);
  const { data: units } = await supabase.from('units').select('id, number, monthly_fee, rooms, board_member').eq('building_id', BID);
  const { data: residents } = await supabase.from('unit_residents').select('unit_id, first_name, last_name, is_primary');
  const { data: building } = await supabase.from('buildings').select('monthly_fee, fee_tiers, board_member_discount').eq('id', BID).single();
  ...
  console.log(`${Object.keys(patterns).length} דפוסי תיאור, ${Object.keys(nameToUnit).length} דפוסי שם\n`);
  ...
  function scoreMatch(tx) {
    ...
    if (key && confirmedPatterns[key]) {
      ...
      if (unit) {
        ...
        return { unitId: cp.unitId, confidence: 'high', reason: `תיאור זהה${dayMatch ? ' + תאריך תואם' : ''}${amountMatch ? ' + סכום תואם' : ''}` };
      }
    }
    ...
    for (const [nameKey, uid] of Object.entries(nameToUnit)) {
      ...
      if (overlap.length >= 2) {
        ...
        if (amountRatio >= 0.7 && amountRatio <= 1.3) {
          return { unitId: uid, confidence: 'high', reason: `שם מאושר (${overlap.join(', ')}) + סכום תואם` };
        }
        return { unitId: uid, confidence: 'medium', reason: `שם מאושר (${overlap.join(', ')}) אבל סכום ${credit} ≠ ${fee}` };
      }
    }
    ...
    for (const unit of (units || [])) {
      ...
      if (parts.length >= 2 && matchingParts.length >= 2) {
        ...
        if (amountRatio >= 0.8 && amountRatio <= 1.2) {
          return { unitId: unit.id, confidence: 'medium', reason: `שם מ-DB (${matchingParts.join(', ')}) + סכום קרוב לתעריף` };
        }
        return { unitId: unit.id, confidence: 'low', reason: `שם מ-DB (${matchingParts.join(', ')}) אבל סכום ${credit} ≠ ${fee}` };
      }
    }
  ...
  for (const tx of unmatched) {
    ...
    const existingMatch = matchedTx.find(t =>
      t.unit_id === unitId && t.month === tx.month
    );
    if (existingMatch) {
    ...
    if (confidence === 'high') {
      ...
      autoMatched++;
      console.log(`  ✓ דירה ${unit.number} | ${m} | ${tx.credit} ₪ | ${reason}`);
    } else {
      // Medium/Low → suggest
      await supabase.from('bank_transactions')
        .update({ unit_id: unitId, match_status: 'suggested' })
        .eq('id', tx.id);
      console.log(`  ? הצעה (${confidence}): דירה ${unit.number} | ${tx.credit} ₪ | ${reason}`);
      suggested++;
    }
  }
  ...
  const { data: allMatched } = await supabase
    .from('bank_transactions').select('unit_id, month, credit')
    .eq('building_id', BID).eq('match_status', 'matched').gt('credit', 0);
  ...
  const { data: existingPayments } = await supabase.from('payments').select('*').eq('building_id', BID);
  ...
  for (const [k, g] of Object.entries(groups)) {
    ...
    if (existing) {
      if (existing.status === 'paid' && Number(existing.amount) >= g.total) { skipped++; continue; }
      const { error } = await supabase.from('payments').update({
        amount: g.total, status,
      ...
      const { error } = await supabase.from('payments').insert({
        building_id: BID, unit_id: g.unitId, amount: g.total,
        month: g.month, status,
        paid_at: new Date().toISOString().split('T')[0],
        method: 'הוראת קבע',
      });
      if (!error) {
        created++;
        console.log(`  + דירה ${unit.number} | ${g.month} | ${g.total}/${fee} ₪ | ${status}`);
      }
      else console.error(`  ✗ ${unit.number} ${g.month}: ${error.message}`);
    }
  }
