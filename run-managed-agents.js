...
`.trim(),
  ...
  {
    // Collection Manager. It may send dunning emails via send_email, but that
    // tool is fail-CLOSED on the building's collection_notifications_enabled
    // toggle — so emails go out ONLY when the toggle is explicitly ON. The
    // toggle is the single on/off control (no code change needed).
