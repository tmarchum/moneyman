...
const SUPABASE_URL =
  process.env.SUPABASE_URL || "https://stncskqjrmecjckxldvi.supabase.co";
...
const existingSet = new Set(
  (existing || []).map(
    (e) => `${e.transaction_date}|${e.description}|${e.credit}|${e.debit}`,
  ),
);
...
const newRows = rows.filter(
  (r) =>
    !existingSet.has(
      `${r.transaction_date}|${r.description}|${r.credit}|${r.debit}`,
    ),
);
