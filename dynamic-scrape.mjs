...
const COLLECTION_PROMPT = (buildingId, year, currentMonth) =>
  `
סקירת גבייה שבועית לבניין ${buildingId}, מ-${year}-01 עד ${currentMonth}.

חוקי יעילות — חובה:
- קרא כל כלי פעם אחת בלבד. אל תחזור על קריאות.
- ניתוח בלבד: אל תשלח מיילים ואל תיצור/תעדכן תיקי גבייה
  (ניהול התיקים הוא דטרמיניסטי בקוד; הכלי upsert_collection_case מנוטרל בשרת).

הגדרות:
- "חוב" = 0 ₪ בחודש מסוים. רק זה.
- סכום שונה מהתעריף = "הפרש לבירור", לא חוב.
...
3. השווה: האם התיקים הפתוחים משקפים את מצב התשלומים בפועל? אתר אי-התאמות,
   מגמות (חייבים חוזרים, הידרדרות/שיפור), ותיקים שכדאי לוועד לטפל בהם השבוע
4. write_alerts — התראה אחת מסכמת: ממצאים, אי-התאמות, והמלצות פעולה לוועד
